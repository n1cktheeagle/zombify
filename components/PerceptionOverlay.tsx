...
type Props = {
  ...
  grid?: { cols: number | null; gutterPx: number | null; confidence: number };
...
export default function PerceptionOverlay({
  ...
  function draw() {
    ...
    const drawBox = (bbox: BBox, lineWidth: number, dash: number[], label?: string) => {
      const scale = displayW / targetWidth;
      const scaled: BBox = [bbox[0] * scale, bbox[1] * scale, bbox[2] * scale, bbox[3] * scale];
    ...
    if (show.blocks && blocks.length) {
      ...
      blocks.forEach((b) => drawBox(b.bbox, 2, [], b.kind || "block"));
    }

    // buttons
    if (show.buttons && buttons.length) {
      ctx.strokeStyle = "#10b981"; // teal
      buttons.forEach((b) => drawBox(b.bbox, 2, [6, 4], "button"));
    }
  ...
  }, [imageUrl, targetWidth, imageNatural.w, imageNatural.h, texts, buttons, blocks, grid, show]);
