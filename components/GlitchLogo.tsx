...
export default function GlitchLogo({ onClick, className = '' }: GlitchLogoProps) {
  ...
  const startGlitch = () => {
    ...
    const interval = setInterval(() => {
      if (glitchCount < maxGlitches) {
        ...
        finalTranslation = randomTranslation; // Store the final translation
        glitchCount++;
      } else {
        // Stop on the last translation instead of reverting
  ...
  const stopGlitch = () => {
    ...
    setIsGlitching(false);
    // Don't reset text - let it persist in whatever language it currently shows
  };
        ...
        {isGlitching && (
          <div className="absolute inset-0 pointer-events-none">
            <div 
              ...
              style={{
                top: `${Math.random() * 100}%`,
                animation: 'glitch-line 0.1s linear infinite'
              }}
            />
            <div 
              ...
              style={{
                top: `${Math.random() * 100}%`,
                animation: 'glitch-line 0.15s linear infinite reverse'
              }}
            />
          </div>
        )}
